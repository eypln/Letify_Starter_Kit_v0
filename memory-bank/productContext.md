# Product Context: Letify

## Var Olma Nedeni
Letify, Malta merkezli bir emlakçı (lettings) platformudur. Ajanların property listing yönetimi, müşteri takibi, gelir yönetimi ve ekip işbirliği süreçlerini tek çatı altında toplar. Ayrıca yapay zeka destekli "Second Brain" sistemi ile sosyal medya içerik stratejisi, öğrenme ve uygulama süreçlerini otomatikleştirir.

## Çözülen Problemler
- **Listing Yönetimi**: Emlak ilanlarının merkezi yönetimi ve takibi
- **Müşteri Takibi**: Kiracı/mal sahibi bilgilerini merkezi bir yerde yönetme
- **Gelir Yönetimi**: Deal'ler, komisyonlar, bonus hesaplamaları
- **Ekip İşbirliği**: Teamwork, assign to agent, collaboration
- **Ödeme Yönetimi**: Abonelik ve billing sistemi
- **AI Second Brain**: Sosyal medya içerik analizi, öğrenme ve bilgi yönetimi otomasyonu
- **Content Engineering**: Çoklu platformdan (Instagram, Facebook, YouTube, TikTok) içerik analizi ve yapılandırılmış bilgi çıkartma

## Nasıl Çalışması Gerektiği
1. **Kullanıcı Girişi**: Güvenli authentication ile sisteme giriş
2. **Dashboard**: Rol bazlı genel bakış (Agent, Teamleader, Manager, Boss)
3. **Listing Management**: Property ekleme, düzenleme, paylaşma
4. **Client Management**: Müşteri profili ve etkileşim takibi
5. **Revenue Management**: Deal kaydı, ödeme takibi, bonus hesaplaması
6. **Teamwork**: Listing ve müşteri paylaşımı
7. **Billing**: Abonelik seçimi ve ödeme
8. **Analytics**: Performans metrikleri ve raporlar
9. **AI Brain Pipeline**: İçerik sınıflandırma → Derin analiz → Bilgi depolama → Semantik arama (RAG)

## Kullanıcı Deneyimi Hedefleri
- **Kolay Kullanım**: Sezgisel interface, minimum öğrenme eğrisi
- **Performans**: Hızlı yükleme, akıcı geçişler
- **Güvenilirlik**: Tutarlı davranış, hata toleransı
- **Ölçeklenebilirlik**: Büyüyen ihtiyaçlara uyum
- **Güvenlik**: Veri koruması ve gizlilik
- **Akıllı İçgörüler**: AI destekli içerik analizi ve öneriler

## Hedef Kullanıcılar
- Malta merkezli emlak (lettings) ajanları
- Ekip liderleri (Teamleader) ve yöneticiler (Manager, Boss)
- Stajyerler (Intern) — görev takibi ve öğrenme
- İçerik stratejistleri — AI Brain ile sosyal medya analizi

## Rekabet Avantajları
- Entegre workflow automation (N8N)
- AI Second Brain (Gemini 2.5 Pro + Notion + Apify) ile content engineering
- Malta lettings pazarına özel bağlamsal AI analiz
- Supabase tabanlı ölçeklenebilir backend
- PWA ile mobil erişim ve push notification