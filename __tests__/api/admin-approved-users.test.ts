...
import { GET } from '@/app/api/admin/approved-users/route'
...
describe('GET /api/admin/approved-users', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  it('returns approved users with auth emails', async () => {
    mockAdminClient.from.mockReturnValue({
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      order: jest.fn().mockResolvedValue({
        data: [{ user_id: 'user-1', full_name: 'Test User', role: 'agent', status: 'approved' }],
        error: null,
      }),
    })
  ...
  it('returns 500 when the approved-user query fails', async () => {
    mockAdminClient.from.mockReturnValue({
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      order: jest.fn().mockResolvedValue({
