...
export default function BossTeamRevenueClient({ user }: { user: User }) {
...
// Agent Payment Dropdown Component
function AgentPaymentDropdown({ 
  revenueId, 
  userId,
  refNo,
  currentStatus, 
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('revenue')
        .update({ agent_payment_status: newStatus })
        .eq('id', revenueId);
      ...
      // If status changed to 'paid', send notification to agent
      if (newStatus === 'paid') {
